...
export default function AddExamButton() {
  ...
  async function save() {
    ...
    const { error } = await supabase.from("exams").insert({ user_id: user.id, subject, exam_date: date, color });
    setSaving(false);
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-4 space-y-3">
              <input value={subject} onChange={(e) => setSubject(e.target.value)}
                placeholder={t.exams_subject_placeholder} className="w-full rounded-lg border border-slate-300 px-3 py-2" />
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setOpen(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-slate-700">
                  {t.exams_cancel}
                </button>
                <button onClick={save} disabled={!subject || !date || saving}
