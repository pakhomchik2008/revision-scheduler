    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <h3 className="font-semibold text-slate-900">
          {selected ? format(selected, "EEEE d MMM") : "Pick a day"}
        </h3>
        {selectedSessions.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">No sessions scheduled.</p>
        ) : (
            ...
            {selectedSessions.map((s) => {
              ...
              return (
                <li key={s.id} className="rounded-lg border border-slate-200 p-2"
                  ...
                  <p className="text-xs text-slate-400">{s.completed ? "Completed" : s.skipped ? "Skipped" : "Pending"}</p>
                </li>
              );
            })}
          </ul>
        ...
        {selectedKey && examDateSet.get(selectedKey) && (
          <p className="mt-3 rounded-lg bg-amber-50 p-2 text-sm text-amber-800">
            ★ {examDateSet.get(selectedKey)!.subject} exam today
          </p>
        )}
      </aside>
    </div>
