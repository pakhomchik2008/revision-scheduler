  ...
  return (
    <div className="mt-6 max-w-lg space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        ...
        <input type="range" min={1} max={8} value={s.daily_study_hours}
          onChange={(e) => setS({ ...s, daily_study_hours: Number(e.target.value) })} className="mt-2 w-full" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-700">{t.settings_start_time}</label>
        <input type="time" value={`${String(s.study_start_hour).padStart(2, "0")}:00`}
          onChange={(e) => setS({ ...s, study_start_hour: Number(e.target.value.split(":")[0]) })}
          className="mt-1 rounded-lg border border-slate-300 px-3 py-2" />
      </div>
      <label className="flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" checked={s.include_weekends}
          onChange={(e) => setS({ ...s, include_weekends: e.target.checked })} />
        {t.settings_include_weekends}
      </label>
      <div>
        <label className="block text-sm font-medium text-slate-700">{t.settings_timezone}</label>
        <input type="text" value={s.timezone} onChange={(e) => setS({ ...s, timezone: e.target.value })}
